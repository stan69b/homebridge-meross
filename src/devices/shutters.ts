...
export class Shutter {
  ...
  parseStatus() {
    if (this.deviceStatus?.payload?.all?.digest?.shutter) {
      ...
      for (let i = 0; i < this.deviceStatus.payload.all.digest.shutter.length; i++) {
        if (this.deviceStatus.payload.all.digest.shutter[i].channel === this.device.channel) {
          isOpen = this.deviceStatus.payload.all.digest.shutter[i].open;
        }
      }
      if (isOpen) {
